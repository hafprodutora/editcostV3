...
const App: React.FC = () => {
  ...
  // Global Timer Engine: Runs if any project has isTimerRunning === true
  useEffect(() => {
      ...
      setProjects(prevProjects => prevProjects.map(p => {
        if (p.isTimerRunning) {
          ...
          if (currentRemaining <= 0) {
            // Timer reached zero
            return { ...p, isTimerRunning: false, pomodoroTimeLeft: 0 };
          }
  ...
  useEffect(() => {
    const runningProject = projects.find(p => p.isTimerRunning);
    if (runningProject && runningProject.pomodoroTimeLeft !== undefined) {
      const formattedTime = formatMinutesToDisplay(runningProject.pomodoroTimeLeft);
      document.title = `${formattedTime} - EditCost`;
    } else {
      document.title = 'EditCost';
    }
  }, [projects]);

  // Sync settings to user-specific local storage
  useEffect(() => {
    if (authEmail) {
  ...
  }, [settings, view, authEmail]);

  // Sync projects to user-specific local storage
  useEffect(() => {
    if (authEmail) {
  ...
  const handleOpenSettings = () => {
    localStorage.setItem('force_settings', 'true');
    setView('onboarding');
  };
  ...
  const handleUpdateProject = useCallback((updated: Project) => {
    setProjects(prev => {
      // Rule: Only one active timer. If updated.isTimerRunning is true, pause all others.
      return prev.map(p => {
        if (updated.isTimerRunning && p.id !== updated.id) {
  ...
  const handleDeleteProject = (id: string) => {
    if (confirm('Deseja realmente excluir este projeto? Esta ação não pode ser desfeita.')) {
  ...
  const renderContent = () => {
    if (!authEmail) {
      if (view === 'signup') {
        return <Signup onSignup={handleSignup} onBackToLogin={() => setView('login')} />;
      }
      return (
        <Login 
          onLogin={handleLogin} 
    ...
    switch (view) {
      ...
      case 'onboarding':
        return <Onboarding settings={settings} onSave={handleSaveSettings} />;
      ...
      case 'dashboard':
        return (
          <div className="space-y-8">
                ...
                <div className="flex items-center gap-4 mb-3 sm:mb-0">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-gray-900 leading-tight">
                      {runningProject.name} <span className="text-gray-400 font-normal">({runningProject.client})</span>
                    </p>
                  </div>
                </div>
            ...
            <SummaryCards 
              totalSeconds={projects.reduce((a, b) => a + b.timeSpentSeconds, 0)}
              totalCost={projects.reduce((a, b) => a + b.totalCost, 0)}
              inProgress={projects.filter(p => p.status === 'Em edição').length}
      ...
      case 'project-detail':
        ...
        return (
          <ProjectDetail 
            ...
            onBack={() => setView('dashboard')}
            onConclude={handleConcludeProject}
          />
        );
      ...
      default:
        return <div>View não implementada.</div>;
    }
  };
